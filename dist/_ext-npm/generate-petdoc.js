...
var indexString = "# IdleLands Pets\n\n";
indexString += "## Table of Contents\n\n";
var petString = "";
var sortedPets = _.sortBy(_.keys(PETDATA));
_.each(sortedPets, function (petKey, index) {
    var pet = PETDATA[petKey];
    ...
    _.each(_.keys(pet.slots), function (slot) {
        petString += "* " + slot + ": " + pet.slots[slot] + "\n";
    });
    petString += "\n";
    petString += "### Special Stats\n";
    _.each(_.keys(pet.specialStats), function (stat) {
        petString += "* " + stat + ": " + pet.specialStats[stat].toLocaleString() + "\n";
    });
    petString += "\n";
    petString += "### Unlock Requirements\n";
    _.each(_.keys(pet.requirements), function (reqType) {
        var reqs = pet.requirements[reqType];
        petString += "#### " + _.upperFirst(reqType) + "\n";
        _.each(_.sortBy(_.keys(reqs)), function (req) {
    ...
    var maxLength = _.max(_.map(_.values(pet.scale), function (array) {
        return _.size(array);
    }));
    petString += "### Upgrades\n";
    ...
    _.each(_.keys(pet.scale), function (upgradeKey) {
        var upgrade = pet.scale[upgradeKey];
        var cost = pet.scaleCost[upgradeKey];
